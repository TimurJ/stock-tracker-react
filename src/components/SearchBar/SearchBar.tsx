...
import { queryFetch, type StockData } from '../../services/queryService'
...
const SearchBar: React.FC<SearchBarProps> = ({ classProp, currentResult }) => {
  const [stocks, setStocks] = useState<StockData[] | undefined>(undefined)
  ...
  useEffect(() => {
    if (stocks?.length && enterPress) {
      const { symbol, description } = stocks[cursor]
      setSelectedResult(description)
      navigate('../' + symbol)
    }
  }, [cursor, enterPress])
